...
> 🎯 **Goal**: Learn to create your own modules, understand `__name__` and `__main__`, and organize code into reusable, well-structured multi-file projects.
...
| Attribute        | Detail                                            |
| ---------------- | ------------------------------------------------- |
| **Level**        | L2: Nerd → Novice                                 |
| **Session**      | 9 of 10 (before Mini Project 2)                    |
| **Duration**     | 30 min                                            |
| **Prerequisites**| L2 Sessions 1-8 (especially S8: File Handling)    |
| **Practice**     | `src/L2/S9/`                                       |
...
1. ✅ Create your own Python modules
2. ✅ Understand `__name__` and `__main__` for module execution
3. ✅ Import from your own modules correctly
4. ✅ Organize code into multiple files effectively
5. ✅ Apply best practices for module design

---

## 📚 Part 1: What Are Modules? (5 min)

### The Problem: Code Gets Big! 🐘
...
## 📚 Part 2: Creating Your First Module (7 min)
...
def divide(first_number, second_number):
    """Divide first_number by second_number safely."""
    if second_number == 0:
        return None
    return first_number / second_number
...
# Method 4: Import all (⚠️ avoid - pollutes namespace)
from math_helpers import *
result = add(1, 2)  # Works but unclear where it came from
```

### 💡 Best Practice: Import Style
...
# ❌ AVOID - Unclear source, potential name conflicts
from math_helpers import *
```

---

## 📚 Part 3: The `__name__` and `__main__` Magic (8 min)
...
# This will run EVERY TIME the file is imported! 😱
...
## 📚 Part 4: Organizing Multi-File Projects (5 min)
...
├── calculator.py     # Calculator-specific functions
...
def main():
    ...
    print("\nResults:")
    display_result("+", first_number, second_number, calculator.add(first_number, second_number))
    display_result("-", first_number, second_number, calculator.subtract(first_number, second_number))
    display_result("×", first_number, second_number, calculator.multiply(first_number, second_number))
    display_result("÷", first_number, second_number, calculator.divide(first_number, second_number))
...
## 📚 Part 5: Module Design Best Practices (3 min)

### 1️⃣ One Purpose Per Module
...
### 4️⃣ Always Include `if __name__ == "__main__"`

```python
# ✅ GOOD - Can test the module directly
if __name__ == "__main__":
    # Test code here
...
### `src/L2/S9/01_creating_modules.py`

Create and use a custom module with string utilities.

### `src/L2/S9/02_name_main.py`

Practice the `__name__` == `"__main__"` pattern.

### `src/L2/S9/03_multi_file_project.py`

Build a multi-file project with proper organization.

**Windows console tip:** These scripts call `sys.stdout.reconfigure(encoding="utf-8")` when supported so emoji lesson headings print without `UnicodeEncodeError` on default `cp1252` terminals. If symbols still look wrong in your terminal, run once per session: `$env:PYTHONIOENCODING = "utf-8"` (PowerShell) before `python src/L2/S9/...`.
...
- File handling (S8)
- Modules and code organization (S9)
...
- **Previous**: [Session 8 - File Handling](S8.md)
- **Next**: [Mini Project 2 - Contact Manager](S10.md)
