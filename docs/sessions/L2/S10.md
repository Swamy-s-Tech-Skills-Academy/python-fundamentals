...
> 🎯 **Goal**: Build a complete contact manager application that reads from and writes to text files, uses well-organized functions, and demonstrates everything learned in Level 2.
...
| S7 | Code Organization | Well-structured, documented code |
| S8 | File I/O | Read/write contacts to text file |
...
1. **📂 Load Contacts** - Read contacts from a text file
2. **💾 Save Contacts** - Write contacts to a text file
3. **➕ Add Contact** - Add new contacts with validation
4. **📋 List Contacts** - Display all contacts
5. **🔍 Search Contacts** - Find contacts by name or phone
6. **🗑️ Delete Contact** - Remove a contact
...
Functions needed:
├── File Operations
│   ├── load_contacts(filename) → list of contacts
│   └── save_contacts(filename, contacts)
│
├── Contact Management
│   ├── add_contact(contacts) → updated contacts
│   ├── delete_contact(contacts, name) → updated contacts
│   └── search_contacts(contacts, query) → matching contacts
│
├── Display Functions
...
> 🧭 **Guided scaffold note:** The `TODO` marker in the starter template below is intentional for learner practice during implementation.  
> ✅ For a complete reference implementation, see `src/L2/S10/contact_manager.py`.
...
def load_contacts(filename):
    ...
    try:
        ...
        print(f"📁 No existing contacts file. Starting fresh!")
...
- ✅ Mastered advanced data structures (sets, tuples)
- ✅ Written elegant list comprehensions
- ✅ Created well-organized functions
- ✅ Handled errors gracefully
- ✅ Read from and written to text files
- ✅ Built a complete, practical application

You're ready for **Level 3: Novice → Object Thinker**! 🚀

---

## 🔗 Related Resources

- **Previous**: [Session 8 - Modules Deep Dive](S9.md)
