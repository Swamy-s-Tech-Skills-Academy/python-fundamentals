...
learning_objectives:
  ...
  - "Learn how Python works behind the scenes (compilation & interpretation)"
  - "Introduction to the Python Interactive Shell (REPL)"
  - "Write and run first Python scripts"
  - "Use built-in functions: print(), input(), type(), and help()"
...
### 💡 **Advanced (Optional): Compiling Python Files Manually**

Most of the time, you **don’t need to think about compilation at all** — Python automatically compiles your `.py` files to bytecode when you run them.  
If you’re curious, you can also ask Python to create bytecode files explicitly:

```powershell
# Compile a single file to bytecode (.pyc) without running it
python -m py_compile src/L1/S1/01_hello.py

# Compile all Python files under src/ (recursively)
python -m compileall src
```

These commands create `__pycache__` folders with `.pyc` files (bytecode).  
For this course, **just running the scripts is enough** — this section is for extra curiosity only.
