...
- ✅ Use built-in functions: print() and input() for basic input/output operations
...
Python comes with many **built-in functions** ready to use without importing anything. In this session, we'll use two essential ones:
...
- ✅ Using built-in functions: `print()` for output and `input()` for user interaction
